...
class StudentRequestController extends Controller
{
    public function newStudentRequest()
    {
        ...
        return view('manager.pages.student.new_tuition_request', compact('studentProfile'));
    }
    ...
    public function approvalStudentList()
    {
        $studentProfile = StudentProfile::with('manager_info')
            ->where(function ($query) {
                $query->where('manager', Auth::guard('manager')->user()->id)
                    ->where(function ($query) {
                        $query->where('student_profile.approval', '=', 1)
        ...
        $manager = 0;
        return view('manager.pages.student.approval_student_list', compact('studentProfile', 'manager_list', 'manager'));
    }
    ...
    public function tuition_search_by_id(Request $req)
    {
        $studentProfile = StudentProfile::with('manager_info')
            ->where('id', $req->job_id)
            ->where('manager', Auth::guard('manager')->user()->id)
            ->orderBy('id', 'DESC')
            ->paginate(10);
        return view('manager.pages.student.search_result_list', compact('studentProfile'));
    }
    ...
    public function rejectedStudentList()
    {
        $studentProfile = DB::table('student_profile')
            ...
            ->orderBy('id', 'DESC')
            ->paginate(10);
        return view('manager.pages.student.rejected_student_list', compact('studentProfile'));
    }
    ...
    public function pendingStudentList()
    {
        $studentProfile = StudentProfile::with('manager_info')
            ->where('manager', Auth::guard('manager')->user()->id)
            ->where('student_profile.approval', '=', 4)
            ->orderBy('id', 'DESC')
            ->paginate(20);

        $manager_list = Manager::where('delete_status', 0)->get();
        $manager = 0;
        return view('manager.pages.student.pending_student_list', compact('studentProfile', 'manager_list', 'manager'));
    }
    ...
    public function cancelStudentList()
    {
        ...
        $manager_list = Manager::where('delete_status', 0)->get();
        $manager = 0;
        return view('manager.pages.student.cancelled_student_list', compact('studentProfile', 'manager_list', 'manager'));
    }
    ...
    public function studentDetails($id)
    {
        ...
        return view('manager.pages.student.student_details', compact('studentDetails', 'teacher', 'applied_teacher', 'confirmed', 'assigned_list', 'rejected_list', 'partner'));
    }
    ...
    public function receiver_list($id)
    {
        ...
        return view('manager.pages.student.receiver_list', compact('teacher'));
    }
    ...
    public function applied_receiver_list($id)
    {
        ...
        return view('manager.pages.student.applied_receiver_list', compact('teacher'));
    }
    ...
    public function find_teacher(Request $req)
    {
        ...
        if ($teacher)
            return view('manager.pages.student.ajax_teacher_profile', compact('teacher', 'student_id', 'confirmed'));
        else
    ...
    public function assign_teacher(Request $req)
    {
        ...
        $assign->assigned_by = Auth::guard('manager')->user()->name;
    ...
    public function confirm_teacher(Request $req)
    {
        ...
        $assign->teacher_id = $req->input('teacher_id');
        $assign->student_id = $req->input('student_id');
        $assign->confirmed_by = Auth::guard('manager')->user()->name;
        $assign->remark = $req->input('remark');
        ...
        if (!empty($req->lead_percentage)) {
            ...
            $student->lead_comission = ($req->salary * $req->lead_percentage) / 100;
    ...
    public function reject_teacher(Request $req)
    {
        ...
        $reject->rejected_by = Auth::guard('manager')->user()->name;
    ...
    //     return view('manager.pages.student.payment_sheet', compact('confirmations', 'manager_list', 'manager', 'total_fee', 'total_discount', 'total_paid', 'total_due', 'manager_name'));
    // }
    ...
    public function payment_sheet()
    {
        ...
        $data = confirmedTeacher::whereHas('student', function ($query) use ($managerId) {
            $query->where('manager', $managerId);
        });

        $total_fee = $data->sum('fee');
        $total_discount = $data->sum('discount');
        $total_paid = $data->sum('paid');
        $total_due = $data->sum('due');

        $confirmations = $data->latest()->paginate(10);
    ...
    {
        if ($req->job_id) {
            $confirmations = confirmedTeacher::where('student_id', $req->job_id)
                ->whereHas('student', function ($q) use ($req) {
                    $q->where('manager', Auth::guard('manager')->user()->id);
                })->latest()->paginate(100);
        } else {
            $confirmations = null;
        }
        ...
        $manager_list = Manager::where('delete_status', 0)->get();
        $manager = 0;
        return view('manager.pages.student.payment_search_result', compact('confirmations', 'manager_list', 'manager'));
    }
    ...
    public function payment_details($id)
    {
        ...
        return view('manager.pages.student.payment_details', compact('studentDetails', 'confirmed', 'transactions'));
    }
    ...
    public function transactions()
    {
        ...
        $transactions = Transaction::where('manager', Auth::guard('manager')->user()->id)->orderBy('created_at', 'DESC')->paginate(30);
        $total = Transaction::where('manager', Auth::guard('manager')->user()->id)->sum('payment');

        $manager_list = Manager::where('delete_status', 0)->get();
        $manager = 0;
        $manager_name = 'Total';

        return view('manager.pages.student.transactions', compact('transactions', 'manager_list', 'manager', 'total', 'manager_name'));
    }
    ...
    public function transactions_search(Request $req)
    {
        ...
        $all_transactions = Transaction::where('manager', Auth::guard('manager')->user()->id)
        ...
        $transactions = $all_transactions->latest()->paginate(30);
        $manager_list = Manager::where('delete_status', 0)->get();
    ...
    public function studentApproval($id)
    {
        ...
        return back()->with('app', 'This Tuition Account Approved');
    }
    ...
    public function studentRejected($id)
    {
        ...
        $tuition->approval = 3;
        $tuition->save();
        return back()->with('rej', 'This Tuition Account Rejected');
    }
